...
import {
  formatExpenditureStatus,
  getExpenditureStatusClasses,
} from "../_lib/helpers";
...
export function ExpenditureStatusPill({ status }: { status: ExpenditureStatus }) {
  return (
    <span
      className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${getExpenditureStatusClasses(status)}`}
    >
      {formatExpenditureStatus(status)}
    </span>
  );
}