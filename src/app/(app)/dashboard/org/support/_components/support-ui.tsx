import { getMessageStatusClass } from "../_lib/helpers";
...
export function MessageStatusPill({ status }: { status: string }) {
  return (
    <span
      className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.08em] ${getMessageStatusClass(status)}`}
    >
      {status.toLowerCase()}
    </span>
  );
